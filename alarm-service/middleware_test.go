...
func TestSemMiddleware_PreflightCaiEm405(t *testing.T) {
	rr := requisicao(t, muxDeTeste(), http.MethodOptions, "/alarms", nil)
...
func TestPreflight_Responde204ComHeaders(t *testing.T) {
	...
	rr := requisicao(t, h, http.MethodOptions, "/alarms", map[string]string{
...
func TestRequisicaoReal_PassaPeloMiddleware(t *testing.T) {
	...
	rr := requisicao(t, h, http.MethodGet, "/alarms", map[string]string{
		"Origin": "http://localhost:5173",
	})

	if rr.Code != http.StatusOK {
		t.Errorf("esperava 200 do handler, obteve %d", rr.Code)
	}
...
func TestOptionsComum_NaoEhTratadoComoPreflight(t *testing.T) {
	...
	rr := requisicao(t, h, http.MethodOptions, "/alarms", nil)
